...
void Unit::SetCurrentHP(float m_currentHealthPoint)
{
    this->m_currentHealthPoint = m_currentHealthPoint;
    OnHPUpdate();
}
void Unit::OnHPUpdate()
{
    switch (m_unitType)
    {
    case Unit::UnitType::Warrior:
        UIManager::Instance().GetUIElementByEnum(UIEnumID::Portrait_Blood_Robin)->adjuster->SetTargetFloat(m_currentHealthPoint / m_maxHealthPoint);
        break;
    case Unit::UnitType::Magician:
        UIManager::Instance().GetUIElementByEnum(UIEnumID::Portrait_Blood_Ursula)->adjuster->SetTargetFloat(m_currentHealthPoint / m_maxHealthPoint);
        break;
    case Unit::UnitType::Healer:
        UIManager::Instance().GetUIElementByEnum(UIEnumID::Portrait_Blood_Hansel)->adjuster->SetTargetFloat(m_currentHealthPoint / m_maxHealthPoint);
        break;
    default:
        break;
    }
...
void Unit::Damaged(Unit* opponentUnit, float opponentDmg)
{
    ...
    SetCurrentHP(m_currentHealthPoint - m_finalHitDamage);
...
void Unit::Damaged(float dmg)
{
    ...
    SetCurrentHP(m_currentHealthPoint - dmg);
...
{
    // 최대 체력이면 x
    SetCurrentHP(m_currentHealthPoint + healingPoint);
