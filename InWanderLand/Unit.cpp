...
void Unit::SetCurrentHp(float p_newHp)
{
    ...
    for (auto unitStatusUI : unitStatusUIs)
    {
        if (!unitStatusUI.expired())
        {
            ...
            if (unitStatusUI.lock()->GetLocalUIsByEnumID().contains(UIEnumID::StatusBar_HP_Number_Max))
            {
                unitStatusUI.lock()->GetLocalUIsByEnumID().at(UIEnumID::StatusBar_HP_Number_Max)->SetNumber(unitTemplateData->pod.max_Health);
            }
        }
    }
    if (!unitStatusPortraitUI.expired())
    {
        ...
        if (unitStatusPortraitUI.lock()->GetLocalUIsByEnumID().contains(UIEnumID::CharInfo_HP_Number_Max))
        {
...
yunutyEngine::coroutine::Coroutine Unit::AttackCoroutine(std::weak_ptr<Unit> opponent)
{
    ...
    float dmg = unitTemplateData->pod.m_autoAttackDamage * (1 + multiplierDamage) + adderAttackDamage;
