...
namespace application
{
    namespace editor
    {
        ...
        void EditorLayer::Initialize()
        {
            ...
            editorModuleList[(int)Module_List::SoundEditor] = &Module_SoundEditor::GetSingletonInstance();
        ...
        void EditorLayer::Update(float ts)
        {
            ...
            if (editorInputControl)
            {
                ...
                if (eim.IsKeyboardPressed(KeyCode::F4))
                {
                    editorModuleList[(int)Module_List::ScriptEditor]->SetActivation(!editorModuleList[(int)Module_List::ScriptEditor]->GetActivation());
                }
                ...
                if (eim.IsKeyboardPressed(KeyCode::F7))
                {
                    editorModuleList[(int)Module_List::SoundEditor]->SetActivation(!editorModuleList[(int)Module_List::SoundEditor]->GetActivation());
                }
        ...
        void EditorLayer::UI_DrawMenubar()
        {
            ...
            if (ImGui::BeginMenu("Module"))
            {
                ...
                bool se = editorModuleList[(int)Module_List::ScriptEditor]->GetActivation();
                if (ImGui::MenuItem("Script Editor", "F4", se))
                {
                    if (se)
                    {
                        editorModuleList[(int)Module_List::ScriptEditor]->SetActivation(false);
                    }
                    else
                    {
                        editorModuleList[(int)Module_List::ScriptEditor]->SetActivation(true);
                    }
                }

                bool ske = editorModuleList[(int)Module_List::Skill]->GetActivation();
                if (ImGui::MenuItem("Skill Editor", "F5", ske))
                {
                    if (ske)
                    {
                        editorModuleList[(int)Module_List::Skill]->SetActivation(false);
                    }
                    else
                    {
                ...
                bool sde = editorModuleList[(int)Module_List::SoundEditor]->GetActivation();
                if (ImGui::MenuItem("Sound Editor", "F7", sde))
                {
                    if (sde)
                    {
                        editorModuleList[(int)Module_List::SoundEditor]->SetActivation(false);
                    }
                    else
                    {
                        editorModuleList[(int)Module_List::SoundEditor]->SetActivation(true);
                    }
                }
