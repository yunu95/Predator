...
namespace application
{
	...
	json FieldPreEncoding(T& classInstance, json& data)
	{
		...
		{
			// Global Constant
			if constexpr (is_GC_v<std::remove_reference_t<decltype(boost::pfr::get<N - 1>(classInstance))>>)
			{
				data[boost::pfr::get_name<N - 1, std::remove_const_t<T>>()] = boost::pfr::get<N - 1>(classInstance).data;
			}
			// Not Pointer, nlohmann json 은 STL Container 변환을 지원함
			else if constexpr (!std::is_pointer_v<std::remove_reference_t<decltype(boost::pfr::get<N - 1>(classInstance))>>)
			{
	...
	json FieldPostEncoding(T& classInstance, json& data)
	{
		...
		else
		{
			...
			{
				data[boost::pfr::get_name<N - 1, std::remove_const_t<T>>()] = boost::pfr::get<N - 1>(classInstance) ? UUID_To_String(boost::pfr::get<N - 1>(classInstance)->GetUUID()) : "";
			}
...
{ \
	json data; \
	application::FieldPreEncoding<boost::pfr::tuple_size_v<Class>>(*this, data); \
	return data; \
}
...
{ \
	application::FieldPreDecoding<boost::pfr::tuple_size_v<Class>>(*this, data); \
	return *this; \
}
