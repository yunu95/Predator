...
public:
    ...
    void Damaged(std::weak_ptr<Unit> opponentUnit, float opponentAp, Vector3d projectileHitPosition);	// 데미지 입었을 경우 추적하는 로직 포함
    ...
    coroutine::Coroutine DamagedEffectCoroutine(std::weak_ptr<Unit> opponent, Vector3d projectileHitPosition);
