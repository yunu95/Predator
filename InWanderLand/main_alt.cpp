...
int main(int, char**)
{
	...
	}
	{
		auto staticMeshObj = yunutyEngine::Scene::getCurrentScene()->AddGameObject();
		staticMeshObj->GetTransform()->SetWorldPosition(yunutyEngine::Vector3d{ 1,3,10 });
		auto staticMesh = staticMeshObj->AddComponent<yunutyEngine::graphics::DirectionalLight>();
		///staticMesh->GetGI().SetLightDiffuseColor();
		staticMeshObj->GetTransform()->SetWorldRotation(yunutyEngine::Vector3d{ 0,0, 0 });
	}
