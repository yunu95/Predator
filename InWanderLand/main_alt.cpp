...
int main(int, char**)
{
	...
	//Vector3d newForward = -staticMeshObj->GetTransform()->GetWorldRotation().Up();
	//Vector3d newUp = staticMeshObj->GetTransform()->GetWorldRotation().Forward();
	//Vector3d newRight = Vector3d::Cross(newUp, newForward);
	//staticMeshObj->GetTransform()->SetWorldRotation(Quaternion::MakeWithAxes(newRight, newUp, newForward));
	auto unitComp = staticMeshObj->AddComponent<CommonUnit>();
	unitComp->Initialize(UnitType::PLAYER);
	//unitComp->SetPlayer(true);
	...
	enemyMesh->GetGI().GetMaterial()->SetPixelShader(L"DebugPS.cso");
	enemyObj->GetTransform()->SetWorldPosition(yunutyEngine::Vector3d{ 12.5, 0, 0 });
	//enemyObj->GetTransform()->SetWorldRotation(Quaternion::MakeWithAxes(newRight, newUp, newForward));
	auto enemyComp = enemyObj->AddComponent<CommonUnit>();
	enemyComp->Initialize(UnitType::WARRIOR);

	// Enemy2
	auto enemyObj2 = yunutyEngine::Scene::getCurrentScene()->AddGameObject();
	auto enemyMesh2 = enemyObj2->AddComponent<yunutyEngine::graphics::StaticMeshRenderer>();
	enemyMesh2->GetGI().LoadMesh("Capsule");
	//enemyMesh->GetGI().SetMesh(L"Capsule");
	enemyMesh2->GetGI().GetMaterial()->SetColor(yunuGI::Color{ 0, 0, 1, 0 });
	//enemyMesh->GetGI().SetShader(0, L"Debug");
	//enemyMesh->GetGI().SetMaterialName(0, L"Forward");
	enemyMesh2->GetGI().GetMaterial()->SetPixelShader(L"DebugPS.cso");
	enemyObj2->GetTransform()->SetWorldPosition(yunutyEngine::Vector3d{ 13.5, 5, 0 });
	//enemyObj->GetTransform()->SetWorldRotation(Quaternion::MakeWithAxes(newRight, newUp, newForward));
	auto enemyComp2 = enemyObj2->AddComponent<CommonUnit>();
	enemyComp2->Initialize(UnitType::ARCHER);

	rtsCam->groundRightClickCallback = [&](Vector3d clickedPosition)
	{
		staticMeshObj->GetComponent<CommonUnit>()->SetDestination(clickedPosition);
	};
