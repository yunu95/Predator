...
void TestCaseNavigationInit()
{
    ...
    delayedTestFunctions->todoList.push_back({ 8,[=]() {
        ...
        Application::Application::GetInstance().AddMainLoopTodo([=]() {
            ...
            Assert::IsTrue((agent->GetTransform()->GetWorldPosition() - Vector3d{7,0,7}).MagnitudeSqr() < 0.3,L"네비게이션 에이전트가 이동을 수행하지 못했습니다.");
            agent->Relocate(Vector3d{ -10,0,0 });
                });
            } });
    delayedTestFunctions->todoList.push_back({ 10,[=]() {
        // 게임 엔진 스레드에서 메인 스레드에서 특정 동작을 구동시키고 싶다면 아래의 AddMainLoopTodo 함수를 사용합니다.
        Application::Application::GetInstance().AddMainLoopTodo([=]() {
            Assert::IsTrue((agent->GetTransform()->GetWorldPosition() - Vector3d{-7.5,0,0}).MagnitudeSqr() < 1,L"네비게이션 에이전트의 Relocate 함수가 순간이동을 수행하지 못했습니다.");
