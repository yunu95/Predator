...
{
	void ChessRook::Init(application::editor::ITemplateData* templateData)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			Vector3d combVector = (i - 2) * gc.chessBlockUnitLength * GetGameObject()->GetTransform()->GetWorldRotation().Forward().Normalized();
			bombObj->GetTransform()->SetLocalPosition(combVector + Vector3d(0, guideUp_Y, (i - 2) * gc.chessBlockUnitOffset));
			...
			Vector3d combVector2 = (i - 2) * gc.chessBlockUnitLength * GetGameObject()->GetTransform()->GetWorldRotation().Right().Normalized();
			bombObj2->GetTransform()->SetLocalPosition(combVector2 + Vector3d((i - 2) * gc.chessBlockUnitOffset, guideUp_Y, 0));
	...
	void ChessRook::OnSummon()
	{
		chessSummonedExplosionDelay = BossSummonChessSkill::pod.chessSummonedExplosionDelay;
		auto templateData = static_cast<application::editor::Interactable_TemplateData*>(ChessPool::Instance().GetRookTemplate());
				...
				lastCoroutine.lock()->PushDestroyCallBack([this]()
					{
						if (GetGameObject()->GetSelfActive())
						{
							ChessPool::Instance().Return(std::static_pointer_cast<ChessRook>(myWeakPtr.lock()));
						}
					});
				isInteracting = true;
			}
	...
	yunutyEngine::coroutine::Coroutine ChessRook::DoInteraction()
	{
		auto& gc = application::GlobalConstant::GetSingletonInstance().pod;
