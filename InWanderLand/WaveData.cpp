...
namespace application
{
    namespace editor
    {
        ...
        void WaveData::PostApplyAsPlaytimeObject()
        {
            ...
            {
                pod.triggerRegion->playtimeRegion->OnEnter.AddCallback([=]() { playtimeWave->ActivateWave(); });
            }
        }
        ...
        void WaveData::HideWaveUnitsVisibility()
        {
            ...
            {
                if (each.first->GetPaletteInstance())
        ...
        void WaveData::ApplyPodAsVector()
        {
            waveUnitDatasVector.clear();
            for (int i = 0; i < pod.waveUnitUUIDS.size(); i++)
            {
                ...
                waveUnitDatasVector.push_back(unit);
            }
        }
