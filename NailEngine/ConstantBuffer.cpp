...
void ConstantBuffer::PushGraphicsData(void* data, unsigned int size)
{
	...
	ResourceBuilder::Instance.Get().device->GetDeviceContext().Get()->VSSetConstantBuffers(0, 1, this->buffer.GetAddressOf());
	ResourceBuilder::Instance.Get().device->GetDeviceContext().Get()->PSSetConstantBuffers(0, 1, this->buffer.GetAddressOf());
