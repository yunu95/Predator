...
void ResourceManager::LoadFBX(const wstring& p_key)
{
...
void ResourceManager::CreateDefaultShader()
{
	...
	// 디퍼드용 셰이더
	{
		Shader* shader = new Shader();
		ShaderInfo info = {};
		info.shaderType = SHADER_TYPE::DEFERRED;
		shader->SetShaderInfo(info);
		shader->CreateGraphicsShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\deferred.hlsli", info);
	...
	// Deferred가 끝나고 실제로 그릴 때 쓰는 셰이더
	{
		...
		info.ps = "PS_Final";
		shader->SetShaderInfo(info);
		shader->CreateGraphicsShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\deferredFinal.hlsli", info);
	...
	{
		Shader* shader = new Shader();
		shader->CreateComputeShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\compute.hlsli", "CS_Main", "cs_5_0");
	...
	// Shadow
	{
		...
		shader->CreateGraphicsShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\shadow.hlsli", info);
	...
	// Shadow Skinned
	{
		...
		shader->CreateGraphicsShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\shadow_skinned.hlsli", info);
	...
	{
		Shader* shader = new Shader();
		shader->CreateComputeShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\computeAnimation.hlsli", "CS_Main", "cs_5_0");
	...
	// 포스트 프로세싱용 셰이더
	{
		...
		shader->CreateGraphicsShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\post.hlsli", info);
	...
	// 파티클 셰이더
	{
		...
		shader->CreateGraphicsShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\particle.hlsli", info);
	...
	{
		Shader* shader = new Shader();
		shader->CreateComputeShader(m_device, m_rootSignature, m_graphicsCommandQueue, m_computeCommandQueue, L"Shader\\particle.hlsli", "CS_Main", "cs_5_0");
...
void ResourceManager::CreateDefaultMaterial()
{
	...
	// 파티클
	{
		...
		Texture* tex = GetTexture(L"Texture\\Particle.png");
